...
void step(uint8_t rep, SReg *seg) {
    ...
    switch (b) {
        ...
        case 0x10: // adc r/m, reg8
            ip += opr1.regrm(&opr2, p, 0, seg);
            val = int8_t(dst = *opr1) + (src = int8_t(*opr2) + int(CF));
            opr1 = setf8(val, dst > uint8_t(val) || (CF && !src));
            return;
        case 0x11: // adc r/m, reg16
            ip += opr1.regrm(&opr2, p, 1, seg);
            val = int16_t(dst = *opr1) + (src = int16_t(*opr2) + int(CF));
            opr1 = setf16(val, dst > uint16_t(val) || (CF && !src));
            return;
        ...
        case 0x14: // adc al, imm8
            ip += 2;
            val = int8_t(dst = AL) + (src = int8_t(p[1]) + int(CF));
            AL = setf8(val, dst > uint8_t(val) || (CF && !src));
            return;
        case 0x15: // adc ax, imm16
            ip += 3;
            val = int16_t(dst = AX) + (src = int16_t(read16(p + 1)) + int(CF));
            AX = setf16(val, dst > uint16_t(val) || (CF && !src));
            return;
        case 0x16: // push ss
            ++ip;
            return push(*SS);
        ...
        case 0x18: // sbb r/m, reg8
            ip += opr1.regrm(&opr2, p, 0, seg);
            val = int8_t(dst = *opr1) - (src = int8_t(*opr2) + int(CF));
            opr1 = setf8(val, dst < uint8_t(src) || (CF && !src));
            return;
        case 0x19: // sbb r/m, reg16
            ip += opr1.regrm(&opr2, p, 1, seg);
            val = int16_t(dst = *opr1) - (src = int16_t(*opr2) + int(CF));
            opr1 = setf16(val, dst < uint16_t(src) || (CF && !src));
            return;
        ...
        case 0x1c: // sbb al, imm8
            ip += 2;
            val = int8_t(dst = AL) - (src = int8_t(p[1]) + int(CF));
            AL = setf8(val, dst < uint8_t(src) || (CF && !src));
            return;
        case 0x1d: // sbb ax, imm16
            ip += 3;
            val = int16_t(dst = AX) - (src = int16_t(read16(p + 1)) + int(CF));
            AX = setf16(val, dst < uint16_t(src) || (CF && !src));
            return;
        case 0x1e: // push ds
            ++ip;
            return push(*DS);
        ...
        case 0x80: // r/m, imm8
            ...
            switch ((p[1] >> 3) & 7) {
                case 0: // add
                    ...
                    return;
                case 1: // or
                    opr1 = setf8(int8_t(*opr1 | CS[ip - 1]), false);
                    return;
                case 2: // adc
                    val = int8_t(dst = *opr1) + (src = int8_t(CS[ip - 1]) + int(CF));
                    opr1 = setf8(val, dst > uint8_t(val) || (CF && !src));
                    return;
                case 3: // sbb
                    val = int8_t(dst = *opr1) - (src = int8_t(CS[ip - 1]) + int(CF));
        ...
        case 0x83: // r/m, imm8 (signed extend to 16bit)
            ...
            switch ((p[1] >> 3) & 7) {
                ...
                case 1: // or
                    ...
                    return;
                case 2: // adc
                    val = int16_t(dst = *opr1) + (src = int16_t(opr2.v) + int(CF));
                    opr1 = setf16(val, dst > uint16_t(val) || (CF && !src));
                    return;
                case 3: // sbb
                    val = int16_t(dst = *opr1) - (src = int16_t(opr2.v) + int(CF));
        ...
        case 0xf7:
            ...
            switch ((p[1] >> 3) & 7) {
                case 0: // test r/m, imm16
                    setf16(int16_t(*opr1 & read16(&CS[ip += 2] - 2)), false);
                    return;
                case 2: // not r/m
                    opr1 = ~*opr1;
                    return;
